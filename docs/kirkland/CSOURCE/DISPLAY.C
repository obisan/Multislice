...
/*  get output device type  */
	...
	printf("display version dated 16-jul-1997 (ejk)\n");
...
/*  Print out single (1D) line as text here  */
	   ...
	   for( i=0; i<npout; i++) {
		...
		x = 1.5 + (nx-1) * xout / a;
		y = 1.5 + (ny-1) * yout / b;
		zpixel = pinter( pix, nx, ny, x, y, 0 );
		if( npix == 2 ) {
		   zipixel = pinter( impix, nx, ny, x, y, 0 );
...
/*
  ...
  ctitle      : character array with title
                   substring split on to differnent lines
...
/*
  ...
  nclev       : (integer) nuber of levels to contour
  xsize,ysize : (real valued) size in inches of output pix
