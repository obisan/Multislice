...
------------------------------------------------------------------------
  ...
  Calculate STEM image using repetetive multislice
...
/*  read in the layer stacking sequence and parse it
	multiple line continuation is sisgnified with a  trailing '-'
...
*/
	...
	almin  = double1D( ndetect, "almin" );
	almax  = double1D( ndetect, "almax" );
	fileout = char2D( ndetect, NCMAX, "fileout" );
	...
	*/
	...
	for( ilayer=0; ilayer<nlayer; ilayer++ ) {
		...
		nx2 = (int) nxl2;
		ny2 = (int) nyl2;
		transr[ilayer] = float2D( nx2,  ny2, "transr" );
		transi[ilayer] = float2D( nx2,  ny2, "transi" );
		...
		if (  ilayer != 0 ) {
			...
			if( ( fabs( ax-ax2 ) > fabs(ABERR*ax) ) ||
				...
				printf("potential    a,b,c = %g, %g, %g",
				       ax2,by2,cz[ilayer] );
				printf("previous pix a,b,c = %g, %g, %g\n",
				       ax,by,cz);
				printf("   layer = %1c\n", cname[ilayer] );
				exit( 0 );
			}
			...
			ax = param[pDX] * nx;
			by = param[pDY] * ny;
			if( (nx != powerof2( nxl2)) || (ny != powerof2( nyl2))) {
	...
	*/
	...
	kxp  = float1D( nxprobe, "kxp" );
	kyp  = float1D( nyprobe, "kyp" );
	kxp2 = float1D( nxprobe, "kxp2" );
	kyp2 = float1D( nyprobe, "kyp2" );
	x2  = float1D( nx, "x2" );
	y2  = float1D( ny, "y2" );
	...
	*/
	propxr = float2D( nlayer, nxprobe, "propxr" );
	propxi = float2D( nlayer, nxprobe, "propxi" );
	propyr = float2D( nlayer, nyprobe, "propyr" );
	propyi = float2D( nlayer, nyprobe, "propyi" );
	...
	k2min = double1D( ndetect, "k2min" );
	k2max = double1D( ndetect, "k2max" );
	...
	totmin =  10.0;
	totmax = -10.0;
	detect = double1D( ndetect, "detect" );
	rmin = float1D( ndetect, "rmin" );
	rmax = float1D( ndetect, "rmax" );

	prober = float2D( nxprobe, nyprobe, "prober" );  /* probe wavefunction */
	probei = float2D( nxprobe, nyprobe, "probei" );
	...
	/*  start here for a full image output */
	   ...
	   for( i=0; i<ndetect; i++) pixr[i] = float2D( nxout, nyout, "pixr" );
