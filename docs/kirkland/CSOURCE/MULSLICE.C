...
void main()
{
	...
	long nxl, nyl, nxl2, nyl2, nbeams, nillum;
	...
	printf("mulslice version dated 28-Jan-1998 ejk\n");
...
/*  get starting value of transmitted wavefunction if required
   (this can only be used in coherent mode)
    remeber to save params for final output pix  */
...
/*  read in atomic potential and specimen parameters
	...
	for( ilayer=0;  ilayer<nlayer; ilayer++ ) {
		...
		nx2 = (int) nxl2;
		ny2 = (int) nyl2;
		transr[ilayer] = float2D( 2*nx2,  ny2, "transr" );
...
*/
	...
	propxr = float2D( nlayer, nx, "propxr" );
	propxi = float2D( nlayer, nx, "propxi" );
	propyr = float2D( nlayer, ny, "propyr" );
	propyi = float2D( nlayer, ny, "propyi" );
